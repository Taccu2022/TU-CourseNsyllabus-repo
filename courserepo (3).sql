...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 17, 2023 at 08:22 AM
...
,`Unit` varchar(20)
,`Syllabus_Objective` varchar(200)
,`Version` varchar(10)
...
INSERT INTO `course_outcome` (`COID`, `Description`, `Course`, `CO1`, `CO2`, `CO3`, `CO4`) VALUES
('COO1', NULL, 'CO208', 'Understand different styles of programming and their differences.', 'Understand the principles of object-oriented problem solving and programming. ', ' Outline the essential features and elements of the C++ programming language.', 'Apply the concepts of class, method, constructor, instance, data abstraction, function\r\nabstraction, inheritance, overriding, overloading, and polymorphism. '),
('COO2', 'NULL', 'CS405', 'Understanding of the basic principles used for achieving the devices and the various functional modules.', 'Knowledge of the various design options and their tradeoffs for the functional units as well as the machine.', 'Understanding of the quantitative performance evaluation of the modules and the machine. ', 'A good understanding of the architectural and organizational aspects of computer systems at the machine level.'),
('COO3', NULL, 'CO214', 'Get sound understanding of the concepts of mathematical logic, sets, relations, recurrence\r\nrelations, functions, graphs, trees.', 'Ability to construct and verify the correctness of mathematical arguments.', 'Ability to solve problems involving recurrence relations and generating functions.', 'Get sound grip on the operations on discrete structures such as sets, functions, relations.'),
('COO4', NULL, 'CS413', 'Clear understanding of the concepts of database systems, their advantages, and\r\napplications.', 'Ability to carry out the conceptual modelling of the data for a given application.', 'Ability to evaluate functional dependencies in the data and model the data in a suitable\r\nnormal form for a relational database.', 'Ability to carry out the database design using appropriate database model and to write\r\noptimized queries. ');
...
('PO001', 'Problem Solving Skills,Technical Competence,System Design and Development,System Design and Development,Effective Communication,Teamwork and Collaboration,Continuous Learning', '', 'CSM'),
('PO002', 'Graduates will be adept at working in interdisciplinary teams, understanding the diverse perspectives of team members and contributing collaboratively to achieve common goals', NULL, 'CSB'),
('PO003', 'Program outcomes for Computer Science and Engineering (CSE) programs can vary depending on the specific curriculum and objectives of the educational institution', NULL, 'CSE'),
('PO004', 'Demonstrate proficiency in core computer science concepts, algorithms, and data structures', NULL, 'CSI'),
('PO005', 'Demonstrate proficiency in programming languages and the ability to write, debug, and optimize code', NULL, 'CSP');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `course_details_view`  AS SELECT `c`.`Course_ID` AS `Course_ID`, `c`.`Course_name` AS `Course_name`, `c`.`Credit` AS `Credit`, concat(`c`.`Lecture`,'L-',`c`.`Tutorial`,'T-',`c`.`Practical`,'P') AS `TeachingMode`, `co`.`CO1` AS `CO1`, `co`.`CO2` AS `CO2`, `co`.`CO3` AS `CO3`, `co`.`CO4` AS `CO4`, `b`.`Book_ID` AS `Book_ID`, `b`.`Book1` AS `Book1`, `b`.`Book2` AS `Book2`, `b`.`Book3` AS `Book3`, `b`.`Book4` AS `Book4`, `u`.`Unit_ID` AS `Unit_ID`, `u`.`unit_1` AS `unit_1`, `u`.`unit_2` AS `unit_2`, `u`.`unit_3` AS `unit_3`, `u`.`unit_4` AS `unit_4`, `u`.`unit_5` AS `unit_5`, `s`.`Syllabus_ID` AS `Syllabus_ID`, `s`.`Unit` AS `Unit`, `s`.`Syllabus_Objective` AS `Syllabus_Objective`, `s`.`Version` AS `Version`, `s`.`Approved_by` AS `Approved_by`, `s`.`Approved_date` AS `Approved_date` FROM ((((`course` `c` left join `course_outcome` `co` on(`c`.`Course_ID` = `co`.`Course`)) left join `book` `b` on(`c`.`Book` = `b`.`Book_ID`)) left join `syllabus` `s` on(`c`.`Syllabus` = `s`.`Syllabus_ID`)) left join `unit` `u` on(`s`.`Unit` = `u`.`Unit_ID`))  ;
